 ...
 */
rt_inline int _seril_virtual_rx(struct rt_serial_device *serial,rt_uint8_t *data,int length){
	int size;
	rt_base_t level;
	int result = RT_EOK;
...
rt_inline int _serial_virtual_tx(struct rt_serial_device *serial, const rt_uint8_t *data, int length)
{
    ...
    if (result == RT_EOK)
    {
        ...
        if (tx_virtual->activated != RT_TRUE)
        {
            ...
            serial->ops->dma_transmit(serial, (rt_uint8_t *)&data, length, RT_SERIAL_VIRTUAL_TX);
        }
        else
        {
            rt_hw_interrupt_enable(level);
                                ...
                                rt_size_t         size)
{
    ...
    {
        return _seril_virtual_rx(serial, buffer, size);
    }
...
void rt_hw_serial_isr(struct rt_serial_device *serial, int event)
{
    switch (event & 0xff)
    {
				...
				case RT_SERIAL_EVENT_TX_VIRTUALDONE:
        {
            ...
            if (rt_data_queue_peek(&(tx_virtual->data_queue), &data_ptr, &data_size) == RT_EOK)
            {
                /* transmit next data node */
                tx_virtual->activated = RT_TRUE;
                serial->ops->dma_transmit(serial, (rt_uint8_t *)&data_ptr, data_size, RT_SERIAL_VIRTUAL_TX);
            }
            else
            {
                tx_virtual->activated = RT_FALSE;
        ...
        case RT_SERIAL_EVENT_RX_VIRTUALDONE:
        {
            ...
            rx_virtual = (struct rt_serial_rx_virtual*)serial->serial_rx;
						rt_uint16_t length = rt_ringbuffer_data_len(&(rx_virtual->ringbuffer));
