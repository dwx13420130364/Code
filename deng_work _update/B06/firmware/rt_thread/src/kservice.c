...
void rt_show_version(void)
{
    ...
    rt_kprintf(" / | \\     %d.%d.%d.%d build %s\n",
               HG_VERSION, HG_SUBVERSION, HG_REVISION,HG_BETA, __DATE__);
...
{
    return vsprintf(buf, format, arg_ptr);
...
rt_device_t rt_console_set_device(const char *name)
{
    ...
    if (new != RT_NULL)
    {
        ...
        /* set new console device */
        if(rt_strcmp("vcom", name) == RT_EOK)
        {
            rt_device_open(new,RT_DEVICE_OFLAG_RDWR| RT_DEVICE_FLAG_VIRTUAL_RX | RT_DEVICE_FLAG_STREAM);
        }
        else
        {
            rt_device_open(new,RT_DEVICE_OFLAG_RDWR| RT_DEVICE_FLAG_INT_RX| RT_DEVICE_FLAG_STREAM);
        }
        _console_device = new;
    }
 ...
 * @param fmt the format
 */
//extern int finsh_lock;
//extern struct rt_messagequeue msg_queue;
...
void rt_kprintf(const char *fmt, ...)
{
    ...
    length = rt_vsprintf(rt_log_buf, fmt, args);
    ...
    else if((_console_device->open_flag & RT_DEVICE_FLAG_STREAM) == RT_DEVICE_FLAG_STREAM)
    {
