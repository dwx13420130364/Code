...
void finsh_set_device(const char *device_name)
{
	...
	if(rt_strcmp("vcom", dev->parent.name ) == RT_EOK)
    {
        ret = rt_device_open(dev,RT_DEVICE_OFLAG_RDWR| RT_DEVICE_FLAG_VIRTUAL_RX | RT_DEVICE_FLAG_STREAM);
    }
    else
    {
        ret = rt_device_open(dev,RT_DEVICE_OFLAG_RDWR| RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_STREAM);
//		ret = rt_device_open(dev,RT_DEVICE_OFLAG_RDWR| RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_DMA_TX);
    }
...
//                if(!finsh_lock)rt_kprintf(FINSH_PROMPT);
                memset(shell->line, 0, sizeof(shell->line));
                shell->line_curpos = shell->line_position = 0;
                break;
            }
